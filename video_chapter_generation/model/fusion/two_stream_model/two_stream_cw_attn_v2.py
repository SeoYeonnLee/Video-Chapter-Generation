...
class ChapterHead(nn.Module):
    ...
    def _init_weights(self):
        """Sequential 모듈과 LayerNorm 모듈을 구분하여 초기화"""
    ...
    '''def _init_weights(self):
        for module in [self.lang_proj_head, self.vision_proj_head, self.fusion_norm]:
            #print(module)
            for layer in module:
                if isinstance(layer, nn.Linear):
                    # ReLU/GELU를 위한 Kaiming 초기화, fan_out 모드 사용
                    nn.init.kaiming_normal_(
                        layer.weight, 
                        mode='fan_out',  # fan_out mode for better gradient flow
                        nonlinearity='relu'  # GELU도 ReLU와 유사하므로 사용
                    )
...
'''
    ...
    def process_clip(self, lang_emb, vision_emb, clip_info):
        ...
        try:
            ...
            # print(f"lang_proj shape: {lang_proj.shape}")
            # print(f"vision_proj shape: {vision_proj.shape}")
            # fusion_emb = self.cross_attention(lang_proj.unsqueeze(1), vision_proj, clip_info)  # [batch, hidden_size]
...
class TwoStream(nn.Module):
    def __init__(self, lang_model, vision_model, lang_embed_size, vision_embed_size,
        ...
        self._lang_feature_cache: Dict[str, torch.Tensor] = {}
        self._vision_feature_cache: Dict[str, torch.Tensor] = {}
        logger.info("Initializing TwoStream model")

    def _compute_cache_key(self, tensor: torch.Tensor) -> str:
        """Compute cache key for a tensor"""
        return f"{tensor.shape}_{hash(tensor.cpu().numpy().tobytes())}"

    def extract_language_features(self, text_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        """Extract and cache language features"""
        cache_key = self._compute_cache_key(text_ids)

        if cache_key not in self._lang_feature_cache:
            with torch.no_grad():
                features = self.lang_model(text_ids, attention_mask)["pooler_output"]
                self._lang_feature_cache[cache_key] = features

        return self._lang_feature_cache[cache_key]

    def extract_vision_features(self, frames: torch.Tensor) -> torch.Tensor:
        """Extract and cache vision features"""
        cache_key = self._compute_cache_key(frames)

        if cache_key not in self._vision_feature_cache:
            with torch.no_grad():
                features = self.vision_model(frames)
                self._vision_feature_cache[cache_key] = features

        return self._vision_feature_cache[cache_key]

    def build_chapter_head(self, output_size):
        print("Starting to build chapter head")
        ...
        print("Fusion head built")

        # Window self attention head
        print("Building window head")
...
'''
    ...
    def forward(self, vision_embeddings, text_ids, attention_mask, clip_info):
        """
        Args:
        ...
        try:
            for i in range(num_clips):
                ...
                # feature extract
                lang_emb = self.extract_language_features(clip_text_ids, clip_attn_mask).to(device)  # [batch_size, hidden_size]
                ...
                # 이미 로드된 vision_embeddings 사용
                frame_emb = vision_embeddings[:, i].to(device) 
                # print(f'Loaded frame emb size: {frame_emb.shape}')
                frame_emb = frame_emb.view(batch_size, self.segment_size, -1).float() # [batch, 16, 2048]
                # print(f'Loaded frame emb size: {frame_emb.shape}')
                # print(f'lang_emb size: {lang_emb.shape}')
    ...
    def clear_cache(self):
            ...
            self._lang_feature_cache.clear()
