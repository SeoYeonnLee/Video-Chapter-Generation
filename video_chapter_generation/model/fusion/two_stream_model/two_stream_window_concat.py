...
class TwoStream(nn.Module):
    ...
    def build_chapter_head(self, output_size):
        ...
        self.window_proj = nn.Sequential(
            nn.Linear(self.hidden_size * (2 * self.window_size), self.hidden_size),
        ...
        for module in [self.window_proj, self.classifier]:
    ...
    def forward(self, img_clip, text_ids, attention_mask, clip_info):
        ...
        try:
            ...
            # Separate target and window embeddings
            target_emb = clip_fusion_embs[target_idx]  # [batch, hidden]
            window_embs = torch.cat([
                torch.stack(clip_fusion_embs[:target_idx], dim=1),  # Left context
                torch.stack(clip_fusion_embs[target_idx+1:], dim=1)  # Right context
            ], dim=1)  # [batch, 2*window_size, hidden]

            # Process window context
            window_embs = window_embs.view(batch_size, -1)  # [batch, 2*window_size*hidden]
            window_embs = self.window_proj(window_embs)  # [batch, hidden]

            # Final prediction
            fusion_embs = torch.cat([target_emb, window_embs], dim=1)  # [batch, 2*hidden]
            binary_logits = self.classifier(fusion_embs)
            binary_prob = F.softmax(binary_logits, dim=-1)

            if not self.training:
                del clip_fusion_embs, window_embs, fusion_embs
                torch.cuda.empty_cache()
    ...
    def configure_optimizers(self, train_config):
        ...
        # train_config.learning_rate = 1e-5, weight_decay = 0.1
        optim_groups = [
            {
                "params": bert_params,
                "lr": train_config.learning_rate,  # 매우 작은 lr
            ...
            },
            {
                ...
                "weight_decay": train_config.weight_decay*0.3,
