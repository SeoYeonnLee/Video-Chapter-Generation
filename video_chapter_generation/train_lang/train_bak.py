...
from data.youtube_dataset import YoutubeClipDataset, YoutubeIntraClipDataset
    ...
    def run_epoch(self, split, epoch, dataset):
        ...
        for i in range(run_time):
            ...
            for it, (img_clip, text_ids, attention_mask, label) in pbar:
                ...
                if len(text_ids.size()) == 3:
                    text_ids = text_ids.view(-1, text_ids.size(2))
                ...
                with torch.set_grad_enabled(is_train):
                    if self.config.data_mode == "text":
                        output = self.model(text_ids, attention_mask, targets=label)
                    else:
                    ...
                    # topk_scores, topk_labels = binary_prob.data.topk(1, 1, True, True)
                    # topk_scores = list(topk_scores.squeeze(1).cpu().numpy())
...
if __name__ == "__main__":
    ...
    # pretrained_ckpt_path = f"/opt/tiger/video_chapter_generation/checkpoint/hugface_bert_constrast_pretrain/batch_256_lr_decay_cosine/pretrain.pth"
    ckpt_path = f"/opt/tiger/video_chapter_generation/checkpoint/video_chapter_hugface_{args.model_type}/batch_{args.batch_size}_lr_decay_{args.lr_decay_type}_cls_on_new_dataloader_intra_clip/checkpoint.pth"
    ...
    train_dataset = YoutubeIntraClipDataset(img_dir, data_file, train_vid_file, tokenizer, clip_frame_num, max_text_len, mode=data_mode)
    ...
    tconf = TrainerConfig(data_mode=data_mode, model_type=args.model_type, max_epochs=args.epoch, batch_size=batch_size, learning_rate=1e-5, block_size=max_text_len,
                        lr_decay_type=args.lr_decay_type, lr_decay=True, warmup_epochs=args.epoch//100, final_epochs=args.epoch//100*9,
