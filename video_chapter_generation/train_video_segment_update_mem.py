...
from model.fusion import two_stream_window_mem
...
class Trainer:
    ...
    def save_checkpoint(self, epoch, best_result):
        ...
        try:
            ...
            del checkpoint
            if self.memory_manager.tracker.should_cleanup():
    ...
    def train(self):
        ...
        for epoch in range(self.config.start_epoch+1, self.config.max_epochs+1):
            ...
            if self.test_dataset is not None and epoch % 30 == 0:
                self.memory_manager.cleanup_if_needed(epoch, is_training=False)
    ...
    def run_epoch(self, split, epoch, dataset):
        ...
        #     print(f"\nInitial GPU {device_id} Memory Status:")
        #     print(f"Allocated: {torch.cuda.memory_allocated(device_id) / 1024**3:.2f}GB")
        #     print(f"Cached: {torch.cuda.memory_reserved(device_id) / 1024**3:.2f}GB")

        # DataParallel 사용 시 각 GPU 초기화
        ...
        losses = []
        loader = DataLoader(dataset, shuffle=is_train, pin_memory=True, batch_size=self.config.batch_size, num_workers=self.config.num_workers)
        pbar = tqdm(enumerate(loader), total=len(loader))

        for it, (img_clip, text_ids, attention_mask, label, clip_info) in pbar:
            try:
                # if it == 0:
                    # print(f"\nBefore forward pass GPU Memory Status:")
                    # for device_id in range(torch.cuda.device_count()):
                    #     print(f"GPU {device_id}: {torch.cuda.memory_allocated(device_id) / 1024**3:.2f}GB")
                ...
                # if it == 0:
                #     print(f"\nAfter forward pass GPU Memory Status:")
                #     for device_id in range(torch.cuda.device_count()):
                #         print(f"GPU {device_id}: {torch.cuda.memory_allocated(device_id) / 1024**3:.2f}GB")
                ...
                if not is_train and self.memory_manager.tracker.should_cleanup():
                    self.memory_manager.cleanup(force=False)
                    if hasattr(self.model, 'module'):
                        for device_id in self.model.device_ids:
                            with torch.cuda.device(device_id):
        ...
        if not is_train:
            # if epoch % 5 == 0 and hasattr(self.model, 'module'):
            #     for device_id in self.model.device_ids:
            #         with torch.cuda.device(device_id):
            #             torch.cuda.empty_cache()
...
if __name__ == "__main__":
    ...
    ckpt_path = f"/home/work/capstone/Video-Chapter-Generation/video_chapter_generation/checkpoint/chapter_localization/temp/checkpoint.pth"
    ...
    if args.data_mode == "text":
        ...
        #     clip_frame_num,
        #     hidden_size)
        model = two_stream_window_mem.TwoStream(
            lang_base_model,
            vision_base_model,
            lang_model.embed_size,
            vision_model.feature_dim,
