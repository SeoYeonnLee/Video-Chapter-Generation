...
if __name__ == "__main__":
    ...
    if args.data_mode == "all":
        if args.location_type == "gt":
            dataset = YoutubeAllChapterTitleDataset(data_file, test_vid_file, tokenizer, max_text_len, chapter_title_text_len)
        else:
            dataset = YoutubeAllChapterTitlePredictDataset(vid2cut_points_file, data_file, test_vid_file, tokenizer, max_text_len, chapter_title_text_len)
    elif args.data_mode == "easy":
        if args.location_type == "gt":
            dataset = YoutubeAllChapterTitleDataset(data_file, test_easy_vid_file, tokenizer, max_text_len, chapter_title_text_len)
        else:
            dataset = YoutubeAllChapterTitlePredictDataset(vid2cut_points_file, data_file, test_easy_vid_file, tokenizer, max_text_len, chapter_title_text_len)
    elif args.data_mode == "hard":
        if args.location_type == "gt":
            dataset = YoutubeAllChapterTitleDataset(data_file, test_hard_vid_file, tokenizer, max_text_len, chapter_title_text_len)
        else:
            dataset = YoutubeAllChapterTitlePredictDataset(vid2cut_points_file, data_file, test_hard_vid_file, tokenizer, max_text_len, chapter_title_text_len)
    else:
    ...
    batch_i = -1
    for text_ids, attention_mask, input_decode_ids, decode_attention_mask, target_decode_ids in infer_loader:
        global_st = time.time()
        batch_i += 1
        ...
        with torch.no_grad():
            ...
            for text_id in text_ids:
                ...
                if len(original_text) <= 0:
                    ...
                    gen_text, sentence_logits = model.generate(original_text, tokenizer, args.gpu)
