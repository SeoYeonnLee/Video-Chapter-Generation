...
class Trainer:
    ...
    def process_val_step(self, binary_prob, it, batch_size, dataset, loss, pbar):
        """Optimized validation step with device handling"""
        with torch.no_grad():
            # binary_prob는 이미 GPU에 있음
            scores = binary_prob[:, 1]  # [batch_size]
            
            # 현재 배치의 점수를 저장
            if not hasattr(self, '_val_scores'):
                self._val_scores = []
                self._val_indices = []
            
            start_idx = it * batch_size
            end_idx = start_idx + scores.shape[0]
            
            # GPU에서의 점수와 인덱스 정보 저장
            self._val_scores.append(scores)  # GPU tensor 그대로 저장
            self._val_indices.append((start_idx, end_idx))
    ...
    def calculate_epoch_metrics(self, dataset, losses, epoch):
        """Optimized metric calculation with proper device handling"""
        try:
            # 모든 점수를 하나의 텐서로 결합 (아직 GPU에 있음)
            ...
            # 비디오별 인덱스 매핑 구성
            vid_to_indices = {}
            for clip_info_idx, clip_info in enumerate(dataset.all_clip_infos):
                vid = clip_info["vid"]
                if vid not in vid_to_indices:
                    vid_to_indices[vid] = {"pred_indices": [], "labels": []}
                
                # 각 비디오별로 예측값 인덱스와 실제 라벨 저장
                vid_to_indices[vid]["pred_indices"].append(clip_info_idx)
                vid_to_indices[vid]["labels"].append(clip_info["clip_label"])
            
            # 비디오별 메트릭 계산
            test_aucs = []
            test_m_aps = []
            
            for vid, info in vid_to_indices.items():
                pred_scores = all_scores[info["pred_indices"]]
                gt_labels = info["labels"]
            ...
            # 임시 저장소 정리
            if hasattr(self, '_val_scores'):
                del self._val_scores
            if hasattr(self, '_val_indices'):
                del self._val_indices
            self.memory_manager.cleanup(force=True)
