...
from model.fusion import two_stream_concat_window_attn
...
class Trainer:
    def __init__(self, model, train_dataset, test_dataset, config):
        ...
        # base_lrs = [1e-5, 1e-5, 3e-4, 2e-4, 1e-4] # BERT, ResNet, Fusion, Window, Classifier

        # self.scheduler = OneCycleLR(
        #     self.optimizer,
        #     max_lr=[lr * 10 for lr in base_lrs],
        #     epochs=config.max_epochs,
        #     steps_per_epoch=len(train_dataset) // config.batch_size,
        #     pct_start=0.15,          # 15% 구간 동안 warmup
        #     anneal_strategy='cos',   # cosine 방식으로 감소
        #     div_factor=10.0,         # 초기 lr = max_lr/10
        #     final_div_factor=1e2     # 최종 lr = 초기 lr/100
        # )
...
if __name__ == "__main__":
    ...
    ckpt_path = f"/home/work/capstone/Video-Chapter-Generation/video_chapter_generation/checkpoint/chapter_localization/concat_simple_window_attn_frame_{args.clip_frame_num}/checkpoint.pth"
    ...
    try:
        ...
        # # language model checkpoint load
        ...
        #         new_key = key.replace('lang_model.', 'base_model.')
        #         new_state_dict[new_key] = value

        #     # 모델 로드 시 누락된 키 무시
        #     missing_keys = lang_model.load_state_dict(new_state_dict, strict=False).missing_keys
        ...
        #     new_state_dict = {}
        #     for key, value in state_dict.items():
        #         new_key = key.replace('vision_model.', 'base_model.')
        #         new_state_dict[new_key] = value

        #     # 모델 로드 시 누락된 키 무시
        #     missing_keys = vision_model.load_state_dict(new_state_dict, strict=False).missing_keys
        ...
        if args.data_mode == "all":
            ...
            model = two_stream_concat_window_attn.TwoStream(
