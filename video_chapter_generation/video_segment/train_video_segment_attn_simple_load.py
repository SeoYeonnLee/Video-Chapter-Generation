'''
concat + simple window attention + npy load
'''
...
def load_vision_embeddings(embedding_dir, video_ids, clip_start_frames, clip_frame_num=16):
        """
        clip_start_frames에 해당하는 vision embedding 로드
        Args:
            embedding_dir (str): Path to the directory containing vision embedding files.
            clip_start_frames (list or tensor): Start frames for which embeddings need to be loaded.
        Returns:
            torch.Tensor: Loaded vision embeddings concatenated along the first dimension.
        """
        embeddings = []
        ...
        for video_id, start_frames in zip(video_ids, clip_start_frames):
            ...
            for start_frame in start_frames:
                ...
                if os.path.exists(embedding_file):
                    emb = np.load(embedding_file)  # npy 파일 로드 # [num_frames, 2048]
                    clip_embeddings.append(torch.tensor(emb, dtype=torch.float32))  # Tensor로 변환
                else:
                    raise FileNotFoundError(f"Embedding 파일을 찾을 수 없습니다: {embedding_file}")
            embeddings.append(torch.stack(clip_embeddings))

        if not embeddings:
            raise ValueError("유효한 임베딩이 로드되지 않았습니다.")
        
        return torch.stack(embeddings, dim=0)  # Embedding 병합
            ...
            try:    
                ...
                for it, batch_data in pbar:
                    try:
                        ...
                        vision_embeddings = load_vision_embeddings(embedding_dir, video_ids, clip_start_frames).to(device)
...
if __name__ == "__main__":
    ...
    ckpt_path = f"/home/work/capstone/Video-Chapter-Generation/video_chapter_generation/checkpoint/chapter_localization/simple_window_batch_{args.batch_size}_frame_{args.clip_frame_num}_v2/checkpoint.pth"
