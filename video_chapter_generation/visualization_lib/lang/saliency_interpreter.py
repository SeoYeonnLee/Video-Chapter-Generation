...
class SaliencyInterpreter:
    ...
    def colorize(self, instance, skip_special_tokens=False):
        ...
        word_cmap = matplotlib.cm.OrRd
        # font_cmap = matplotlib.cm.Reds
        ...
        colored_string += template.format("#ffffff", " | gt label: {} ".format(instance['gt_label']))
        colored_string += template.format("#ffffff", " | pred label: {} ".format(instance['pred_label']))
    ...
    def forward_step(self, batch):
        ...
        img_clip, text_ids, attention_mask, gt_label = batch
        logits, prob = self.model(text_ids, attention_mask)
        loss = F.cross_entropy(logits, gt_label)
        pred_label = torch.argmax(logits, dim=1)

        self.batch_output = [text_ids, pred_label, gt_label]
    ...
    def update_output(self):
        ...
        input_ids, pred_label, gt_label, grads = self.batch_output

        tokens = [self.tokenizer.convert_ids_to_tokens(input_ids_)[1:] for input_ids_ in input_ids]

        grads = grads[:, 1:, :]
        grads = torch.abs(grads)
        embedding_grads = grads.sum(dim=2)
        # embedding_grads[:, 0] = 0       # ignore the gradient in [CLS] token
        ...
        batch_output = []
        iterator = zip(tokens, pred_label, gt_label, embedding_grads)

        for example_tokens, example_pred_label, example_gt_label, example_grad in iterator:
            example_dict = dict()
            # as we do it by batches we has a padding so we need to remove it
            example_tokens = [t for t in example_tokens if t != self.tokenizer.pad_token]
            example_dict['tokens'] = example_tokens
            example_dict['grad'] = example_grad.cpu().tolist()[:len(example_tokens)]
            example_dict['pred_label'] = example_pred_label.item()
            example_dict['gt_label'] = example_gt_label.item()
            batch_output.append(example_dict)
        return batch_output
